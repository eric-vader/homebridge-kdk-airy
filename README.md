...
Releasing: `make bump` (or `make bump BUMP=minor`) bumps the version, commits, tags and pushes; then `make release`, with `NOTES="..."` or `NOTES_FILE=path` for release notes above the generated ones. It creates the GitHub release
